...
public class AccountsService {
  ...
  public static final short MAX_AUTHENTICATION_ATTEMPTS = 5;
  ...
  private void sendEmailVerification(Account savedAccount, Account account) {
    ...
    String activationLink =
        appConfig.getActivationBaseUrl() + savedAccount.getEmailVerificationToken();
    Map<String, Object> variables =
        Map.of("username", savedAccount.getUsername(), "activationLink", activationLink);
    try {
      mailService.sendTemplate(
          savedAccount.getUsername(), savedAccount.getEmail(), "activation", variables);
  ...
  public Account checkCredentials(String email, String password) throws AuthenticationException {
    Account account =
        accountsRepository
            ...
            .orElseThrow(
                () -> new AuthenticationException(AuthenticationExceptionType.INVALID_CREDENTIALS));
    ...
    if (account.getAuthenticationAttempts() >= MAX_AUTHENTICATION_ATTEMPTS) {
      throw new AuthenticationException(
          AuthenticationExceptionType.MAX_AUTHENTICATION_ATTEMPTS_LOCK);
    }
